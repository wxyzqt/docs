...
提供的`JSON Schema`在路由解析 json 方面的表现相当好；
...
fastify 有自己的命令行工具[Fastify-CLI](https://www.npmjs.com/package/fastify-cli)，cli 生成的项目交由 fastify 启动。

其配置项`--config`接受一个配置文件，要注意的是，仅接收`cjs`文件，以下是其源码库中不同文件摘取的相关内容。
...
可以看出，仅通过函数内的`require`接受配置文件，且要求配置文件的导出方式仅能通过顶层`module.exports`导出，实践验证`module.exports`也不能通过函数动态返回导出配置。
