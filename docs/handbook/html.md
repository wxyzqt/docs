...
添加任一属性时，遇到 js 文件会异步下载，不会阻碍后续操作

添加任一属性的 js 文件下载完成后，async 属性的文件会立即执行，defer 属性的文件则会在文档被解析后，但在触发 DOMContentLoaded 事件之前执行
...
搜索引擎收集信息的规则中，一般比较注重 meta 标签内的头部信息，在页面则有 h1、h2 等标签，这种时候明确的标签比纯粹的 div 在权重上更具优势
...
对于特定的单页应用来说，服务端渲染 SSR，网站 sitemap 则是 seo 的首选方式
...
没有 css 样式表的情况下，页面呈现内容结构的外观来源于浏览器内置的 style 样式表以及标签内置 css 设置
:::
